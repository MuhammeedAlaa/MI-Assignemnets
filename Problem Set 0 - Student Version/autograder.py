...
class Problem:
    def __init__(self, **kwargs) -> None:
        ...
        if "function" in kwargs: self.default_fn = load_function(kwargs["function"])
        self.default_cmp = default_comparator
        if "comparator" in kwargs: self.default_cmp = load_function(kwargs["comparator"])
    ...
    def run(self):
        ...
        for test_index, test_case in enumerate(test_cases):
            ...
            fn = self.default_fn
            if "function" in test_case: fn = load_function(test_case["function"])
            ...
            if "comparator" in test_case: cmp = load_function(test_case["comparator"])
            cmp_args = Arguments(
