...
def extract_utilities(env: GridEnv, agent: ValueIterationAgent[Point, Direction]):
    states = env.mdp.walkable - env.mdp.terminals
    return {state:agent.utilities.get(str(state), 0) for state in states}
...
def extract_q_values(env: GridEnv, agent: ValueIterationAgent[Point, Direction]):
    states = env.mdp.walkable - env.mdp.terminals
